...
1)
  ...
  echo -e "${CYAN}soft by perfnode — всё готово для запуска.${NC}"
  echo -e "${PURPLE}-----------------------------------------------------------------------${NC}"
  ;;
...
2)
  ...
  echo -e "${PURPLE}-----------------------------------------------------------------------${NC}"
  echo -e "${RED}Пополните минимум 0.2 ETH (Sepolia) на адрес:${NC}"
  echo -e "${CYAN}${ETH_ADDRESS}${NC}"
  echo -e "${RED}и убедитесь на etherscan.io, что средства зачислены.${NC}"
  echo -ne "${YELLOW}После этого нажмите Enter для продолжения...${NC}"; read -r _

  echo -ne "${YELLOW}Введите приватный ключ от старого валидатора (без 0x): ${NC}"; read -r OLD_VALIDATOR_PK
  echo -ne "${YELLOW}Введите адрес вывода стейка: ${NC}"; read -r WITHDRAW_ADDR
  ...
  echo -ne "${YELLOW}Публичный IP [${DETECT_IP}]: ${NC}"; read -r P2P_IP
  ...
  echo -e "${PURPLE}Адрес аттестера: ${CYAN}${ETH_ADDRESS}${NC}"
  echo -ne "${YELLOW}После проверки нажмите Enter...${NC}"; read -r _

  # Создание keystore.json
...
EOF
  ...
  echo -e "${PURPLE}-----------------------------------------------------------------------${NC}"
  echo -e "${GREEN}soft by perfnode — Aztec-нода успешно запущена!${NC}"
  echo -e "${CYAN}perfnode.tech ✦ Stay synced.${NC}"
  echo -e "${PURPLE}-----------------------------------------------------------------------${NC}"
  sleep 2
  $COMPOSE logs -fn 200
  ;;
...
5)
  ...
  if [ "$CONFIRM" = "YES" ]; then
    ...
    echo -e "${GREEN}soft by perfnode — удаление завершено.${NC}"
  else
